...
#Specify data path
# veDataPath <- here("Results\\ices_vms_lb_datacall_2024_outputs\\table1Save_2020_2023.csv")
# 
# leDataPath <- here("Results\\ices_vms_lb_datacall_2024_outputs\\table2Save_2020_2023.csv")
veDataPath <- here("Results\\ices_vms_lb_datacall_2024_outputs\\table1Save_2015_2023.RData")

leDataPath <- here("Results\\ices_vms_lb_datacall_2024_outputs\\table2Save_2015_2023.RData")
...
get ( load ( file ) |> tidyr::as_tibble() |>select ( value  ) |>  pull( )  ) %>% 
    dplyr::rename(type = 1,
                  country = 2,
                  year = 3,
                  month = 4,
...
get ( load ( file ) |> tidyr::as_tibble() |>select ( value  ) |>  pull( )  ) %>% 
    dplyr::rename(type = 1,
                  ...
                  n_records = 14, 
                  speed = 15,
                  effort = 16,             # units in hours
                  int = 17,                # average interval
                  length = 18,             # average vessel length
                  kw = 19,                 # average kw
                  kwh = 20,                # kw x effort
                  sa = 21, 
                  catch = 22,
